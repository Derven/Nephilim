...
/mob/living/human
	...
	proc/death()
		...
		message_to_usr("<font size='3' color='#696969'>Наступила смерть")
		nocontrol()
	...
	process()
		...
		if(!oheart)
			...
			if(prob(45))
				if(!rest)
					rest()
				message_to_usr("<font size='5' color='red'>Вы чувствуете острую боль в области сердца</font>")
		else
			if(oheart.temp <= 0)
				health -= rand(1,5)
				if(prob(45))
		...
		if(!vector || !strength)
			for(var/obj/hud/rhand/RH in usr.client.screen)
				if(RH.SLOT != null)
					...
					if(RH.active == 1)
						usr.call_message(3, "<font size='3' color='#696969'>Бросает [itemname] на пол")
						RH.remove_from_slot()
			...
			for(var/obj/hud/rhand/RH in usr.client.screen)
				if(RH.SLOT != null)
					var/itemname = RH.SLOT.ru_name
					if(RH.active == 1)
						usr.call_message(3, "<font size='3' color='#696969'>Бросает [itemname] прочь")
